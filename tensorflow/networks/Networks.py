...
class Networks:
    ...
    def encoder_fc(self, input_data, num_classes, is_training=False):
      ...
      with tf.variable_scope('encoder_fc', reuse=not is_training):
        ...
        x = tf.nn.conv2d(32, 3,2, padding="same")(x)
        x = tf.nn.relu(x)
        ...
        x = tf.nn.max_pool((2, 2))(x)  
        x = tf.nn.dropout(0.5)(x)
        x = tf.nn.conv2d(64,3,2, padding="same")(x)
        x = tf.nn.relu(x)
        x = tf.nn.batch_normalization()(x)
        x = tf.nn.max_pool((2, 2))(x) 
        x = tf.nn.dropout(0.5)(x)
    ...
    def output_from_gaussian(self, gaussian, output_size, is_training=False):    
      ...
      with tf.variable_scope('decoder_gauss', reuse=not is_training):                
        ...
        X = tf.nn.conv2d_transpose(64, (3, 3),2, padding="same")(x)
        x = tf.nn.relu(X)
        x = tf.nn.batch_normalization()(x)
        x = tf.keras.layers.UpSampling2D((2, 2))(X)
        X = tf.nn.conv2d_transpose(64, 3,2, padding="same")(x)
        x = tf.nn.relu(X)
        x = tf.nn.batch_normalization()(x)
        x = tf.keras.layers.UpSampling2D((2, 2))(x)
        X = tf.nn.conv2d_transpose(32, 3,2, padding="same")(x)
        x = tf.nn.relu(X)
        x = tf.nn.batch_normalization()(x)
        x = tf.keras.layers.UpSampling2D((2, 2))(x)
        X = tf.nn.conv2d_transpose(32, 3,2, padding="same")(x)
        x = tf.nn.relu(X)
        x = tf.nn.batch_normalization()(x)
        out = tf.nn.conv2d_transpose(3, 3,2, padding="same")(x)
        x = tf.nn.relu(out)
